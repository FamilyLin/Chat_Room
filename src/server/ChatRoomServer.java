...
import static common.Utils.isValidUserName;
...
public class ChatRoomServer {
    ...
    class ClientHandler implements Runnable {
        ...
        private void handleChatMessage(DataExchange dataExchange, ChatMessage chatMessage, String toName) {
            ...
            if (toEx == null) {
                dataExchange.send(new ChatMessage(ADMIN_NAME, chatMessage.getFrom(), "用户名\""
                        + toName + "\"不存在"));
            } else {
        ...
        private void handleServerCommand(ChatMessage chatMessage) {
            ...
            if (command.equalsIgnoreCase(SERVER_COMMAND_LOGOFF)) {
                ...
                from.send(new ChatMessage(ADMIN_NAME, chatMessage.getFrom(), "所有在线用户："
                        + allNames));
            }
        }
